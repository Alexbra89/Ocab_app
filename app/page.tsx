...
const supabase = createClient(
  'https://lentvokmursalarffetv.supabase.co',
  'sb_publishable_vS5Fx5PRE_cH4Ch4TAH-Uw_U6ymahic',
);
...
interface Prosjekt {
  ...
  utstyr: number;
  notat: string;
  status: 'Befaring utført' | 'Skadedyr behandlet' | 'Forebyggende utført';
  dato: string;
  bilder: string[];
  faktiskTimer?: number;
  rabatt?: number;
  tilbudGyldigTil?: string;
  ansvarlig: 'Alexander' | 'Ståle' | 'Baard' | 'Arkiv';
  kjoreKm?: number;
  bompenger?: number;
}

export default function OcabProV7() {
  ...
  const [aktivMedarbeider, setAktivMedarbeider] = useState<'alle' | 'Alexander' | 'Ståle' | 'Baard' | 'Arkiv'>('alle');
  ...
  const [ansvarlig, setAnsvarlig] = useState<'Alexander' | 'Ståle' | 'Baard' | 'Arkiv'>('Arkiv');
  ...
  const STATUS_COLORS = {
    ...
    'Skadedyr behandlet': { bg: '#cce5ff', border: OCAB_BLA, text: '#004085' },
    'Forebyggende utført': { bg: '#d1e7dd', border: '#198754', text: '#0f5132' }
  };
  ...
  // ===== EKSISTERENDE APP LOGIKK =====
  useEffect(() => {
    const lagret = localStorage.getItem('ocab_v7_data');
    if (lagret) setProsjekter(JSON.parse(lagret));
  }, []);

  useEffect(() => {
    localStorage.setItem('ocab_v7_data', JSON.stringify(prosjekter));
  }, [prosjekter]);
  ...
  const apneRedigering = (p: Prosjekt) => {
    ...
    setAnsvarlig(p.ansvarlig || 'Arkiv');
  ...
  const resetSkjema = () => {
    setRedigererId(null);
    setKunde(''); setMengde(0); setTimer(0); setUtstyr(0); setNotat(''); setFastprisSum(0); setBilder([]); setFaktiskTimer(0); setRabatt(0); setTilbudGyldigTil(''); setAnsvarlig('Arkiv'); setKjoreKm(0); setBompenger(0);
  };
  ...
  const flyttTilMedarbeider = (e: React.MouseEvent, prosjektId: number, nyAnsvarlig: 'Alexander' | 'Ståle' | 'Baard' | 'Arkiv') => {
    e.stopPropagation();
...
<head>
  ...
  <style>
    ...
    .logo-placeholder { width: 150px; height: 80px; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center; color: #999; font-size: 0.8rem; }
...
<body>
  <div class="header">
    ...
    <div class="logo-placeholder">
      LOGO HER<br>(Last opp i nettleser)
    </div>
  </div>
  ...
  <div class="footer">
    ...
    <p>Org.nr: [DITT ORG.NR] | Tlf: [DIN TELEFON] | E-post: [DIN E-POST]</p>
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '15px', backgroundColor: '#f4f7f6', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <header style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', marginBottom: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '8px', marginBottom: '15px' }}>
          ...
          <div 
            onClick={() => setAktivMedarbeider('Arkiv')} 
            style={{ 
              ...medarbeiderBoxStyle, 
              backgroundColor: aktivMedarbeider === 'Arkiv' ? '#95a5a6' : '#f9f9f9',
              color: aktivMedarbeider === 'Arkiv' ? 'white' : '#333',
              borderColor: '#95a5a6'
            }}
          >
            <div style={{ fontSize: '1.3rem', fontWeight: 'bold' }}>{medarbeiderStats('Arkiv').antall}</div>
            <div style={{ fontSize: '0.7rem', marginTop: '2px' }}>📦 Arkiv</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={filterSelectStyle}>
            ...
            <option value="Skadedyr behandlet">Skadedyr behandlet</option>
            <option value="Forebyggende utført">Forebyggende utført</option>
          </select>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {filtrerteProsjekter.map(p => {
          ...
          return (
            <div key={p.id} onClick={() => apneRedigering(p)} style={{ 
              ...
              <div style={{ 
                ...
                {p.status === 'Skadedyr behandlet' && '🐛'} 
                {p.status === 'Forebyggende utført' && '🛡️'}
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px', paddingRight: '120px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.7rem', color: '#666', marginTop: '2px' }}>{p.dato}</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '5px', marginTop: '10px' }}>
                {['Befaring utført', 'Skadedyr behandlet', 'Forebyggende utført'].map((s) => (
                  <button key={s} onClick={(e) => { e.stopPropagation(); setProsjekter(prosjekter.map(proj => proj.id === p.id ? {...proj, status: s as any} : proj))}}
                    style={{ 
                      flex: 1, 
                      fontSize: '0.65rem', 
                      padding: '6px 2px', 
                      borderRadius: '6px', 
                      border: `1px solid ${STATUS_COLORS[s as keyof typeof STATUS_COLORS].border}`, 
                      backgroundColor: p.status === s ? STATUS_COLORS[s as keyof typeof STATUS_COLORS].border : 'white', 
                      color: p.status === s ? 'white' : '#666',
                      cursor: 'pointer' 
                    }}>
                    {s === 'Befaring utført' ? '📋' : s === 'Skadedyr behandlet' ? '🐛' : '🛡️'}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: '5px', marginTop: '8px' }}>
                ...
                {['Alexander', 'Ståle', 'Baard', 'Arkiv'].map((person) => (
                  <button 
                    key={person}
      ...
      <footer style={{ textAlign: 'center', marginTop: '30px', padding: '15px', color: '#999', fontSize: '0.8rem' }}>
        ...
        <div style={{ fontSize: '0.7rem', marginTop: '5px' }}>v7.0 med Supabase Auth</div>
      </footer>
    </div>
  );
}
